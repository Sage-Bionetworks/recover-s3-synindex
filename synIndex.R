...
if (SCRIPT_PROCESS=='sts_synindex') {
  ###########
  ## Get a list of all files to upload and their synapse locations(parentId) 
  ###########
  STR_LEN_AWS_DOWNLOAD_LOCATION = stringr::str_length(AWS_PARQUET_DOWNLOAD_LOCATION)
  ...
  synapse_fileview <- synapser::synTableQuery(paste0('SELECT * FROM ', PARQUET_FILEVIEW_ID))$asDataFrame()
